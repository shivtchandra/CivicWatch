...
const SafetyAlerts = () => {
  ...
  useEffect(() => {
    if (!user) {
      toast({
        ...
        description: "You need to log in to view safety alerts.",
        variant: "destructive",
      });
      navigate("/auth");
    }
  ...
  const safetyOnlyAlerts = safetyAlerts.filter(
    (sa) => sa.type !== "missing_person" && sa.type !== "lost_found"
  );
  ...
  return (
    <div className="min-h-screen bg-minimal">
      ...
      <main className="container mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold mb-8 text-silver">Safety Alerts</h1>
        <ReportsSection
          loading={loading}
          currentReports={safetyOnlyAlerts}
          activeTab="safety"
